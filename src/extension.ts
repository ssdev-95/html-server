...
export function activate(context: vscode.ExtensionContext) {
	...
	let disposable = vscode.commands.registerCommand('html-server.helloWorld', () => {
		// The code you place here will be executed every time your command is executed
		// Display a message box to the user
		vscode.window.showInformationMessage('Hello World from html-server!');
	});

	let lol = vscode.commands.registerCommand('Hi', () => {
		const workspace = vscode.workspace.getWorkspaceFolder.name;
		vscode.window.showWarningMessage(workspace);
	});

	context.subscriptions.push(disposable);
	context.subscriptions.push(lol);
}
